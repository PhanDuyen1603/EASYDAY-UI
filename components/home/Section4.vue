<template>
  <section class="margin-page">
    <div class="mt-60">
      <div class="elementor-posts">
        <h2 class="title-posts">Foods Idea</h2>
        <div class="bottom-title"></div>
      </div>
      <b-container class="recommend-food-main d-flex">
        <b-col v-for="item in ides1" :key="item.id" class="recommend-foods">
          <NuxtLink :to="{ name: 'product-detail-slug', params: { slug: item.slug } }">
            <img
              class="img-idea"
              width="275"
              height="275"
              :src="`/images/ideas/${item.img}.png`"
            />
            <div class="recommend-food-content">
              <div class="title">{{ item.name }}</div>
              <div data-v-b3340884="" class="add-to-cart-wrap padding-idea">
                <a data-v-b3340884="" href="/" data-quantity="1"
                  ><span data-v-b3340884=""
                    ><svg
                      data-v-b3340884=""
                      version="1.1"
                      xmlns="http://www.w3.org/2000/svg"
                      xmlns:xlink="http://www.w3.org/1999/xlink"
                      x="0px"
                      y="0px"
                      viewBox="0 0 500 500"
                      enable-background="new 0 0 500 500"
                      xml:space="preserve"
                      class="qodef-svg"
                      style="width: 22px; margin-right: 10px"
                    >
                      <g data-v-b3340884="">
                        <path
                          data-v-b3340884=""
                          d="M448.2,292.3c-5.6-0.6-10.9,3.3-11.6,9l-21.8,168.6c-0.6,5.3-5,9.3-10.3,9.3h-310c-5.3,0-9.8-4-10.4-9.6
      L62.3,302.1c-0.7-5.7-6-9.7-11.6-9c-5.7,0.7-9.7,5.9-9,11.6l21.7,167.3c1.7,15.9,15,27.9,31,27.9h310c16,0,29.3-12,31-27.7
      L457.2,304C457.9,298.3,453.9,293.1,448.2,292.3z"
                        ></path>
                        <path
                          data-v-b3340884=""
                          d="M166.3,447.8V323.4c0-5.7-4.6-10.4-10.4-10.4c-5.7,0-10.4,4.6-10.4,10.4v124.4c0,5.7,4.6,10.4,10.4,10.4
      C161.6,458.2,166.3,453.6,166.3,447.8z"
                        ></path>
                        <path
                          data-v-b3340884=""
                          d="M353.3,447.8V323.4c0-5.7-4.6-10.4-10.4-10.4c-5.7,0-10.4,4.6-10.4,10.4v124.4c0,5.7,4.6,10.4,10.4,10.4
      C348.6,458.2,353.3,453.6,353.3,447.8z"
                        ></path>
                        <path
                          data-v-b3340884=""
                          d="M290.9,447.8V323.4c0-5.7-4.6-10.4-10.4-10.4s-10.4,4.6-10.4,10.4v124.4c0,5.7,4.6,10.4,10.4,10.4
      S290.9,453.6,290.9,447.8z"
                        ></path>
                        <path
                          data-v-b3340884=""
                          d="M228.6,447.8V323.4c0-5.7-4.6-10.4-10.4-10.4c-5.7,0-10.4,4.6-10.4,10.4v124.4c0,5.7,4.6,10.4,10.4,10.4
      C224,458.2,228.6,453.6,228.6,447.8z"
                        ></path>
                        <path
                          data-v-b3340884=""
                          d="M468.4,188.4h-43.6L278.7,46.1c1.8-3.9,2.8-8.1,2.8-12.7c0-16.8-13.8-30.5-30.7-30.5s-30.7,13.7-30.7,30.5
      c0,4.5,1,8.8,2.8,12.7L76.9,188.4H32.1C15,188.4,1,202.4,1,219.5v20.8c0,17.2,14,31.2,31.2,31.2h436.3c17.2,0,31.2-14,31.2-31.2
      v-20.8C499.6,202.4,485.6,188.4,468.4,188.4z M250.9,23.7c5.5,0,9.9,4.4,9.9,9.8c0,2.6-1.1,5-2.8,6.8c0,0,0,0-0.1,0.1
      c0,0,0,0.1-0.1,0.1c-1.8,1.8-4.2,2.8-7,2.8c-2.7,0-5.2-1.1-7-2.8c0,0,0-0.1-0.1-0.1c0,0,0,0-0.1-0.1c-1.7-1.8-2.8-4.1-2.8-6.8
      C241,28,245.4,23.7,250.9,23.7z M237.5,60.9c4,1.9,8.5,3.1,13.3,3.1c4.8,0,9.3-1.1,13.3-3.1L395,188.4H106.7L237.5,60.9z
       M478.8,240.3c0,5.7-4.7,10.4-10.4,10.4H32.1c-5.7,0-10.4-4.7-10.4-10.4v-20.8c0-5.7,4.7-10.4,10.4-10.4h436.3
      c5.7,0,10.4,4.7,10.4,10.4V240.3z"
                        ></path>
                      </g></svg
                  ></span>
                  Add to cart</a
                >
              </div>
            </div>
          </NuxtLink>
        </b-col>
      </b-container>
      <b-container class="recommend-food-main d-flex">
        <b-col v-for="item in ides2" :key="item.id" class="recommend-foods">
          <NuxtLink :to="{ name: 'product-detail-slug', params: { slug: item.slug } }">
            <img
              class="img-idea"
              width="275"
              height="275"
              :src="`/images/ideas/${item.img}.png`"
            />
            <div class="recommend-food-content">
              <div class="title">{{ item.name }}</div>
              <div data-v-b3340884="" class="add-to-cart-wrap padding-idea">
                <a data-v-b3340884="" href="/" data-quantity="1"
                  ><span data-v-b3340884=""
                    ><svg
                      data-v-b3340884=""
                      version="1.1"
                      xmlns="http://www.w3.org/2000/svg"
                      xmlns:xlink="http://www.w3.org/1999/xlink"
                      x="0px"
                      y="0px"
                      viewBox="0 0 500 500"
                      enable-background="new 0 0 500 500"
                      xml:space="preserve"
                      class="qodef-svg"
                      style="width: 22px; margin-right: 10px"
                    >
                      <g data-v-b3340884="">
                        <path
                          data-v-b3340884=""
                          d="M448.2,292.3c-5.6-0.6-10.9,3.3-11.6,9l-21.8,168.6c-0.6,5.3-5,9.3-10.3,9.3h-310c-5.3,0-9.8-4-10.4-9.6
      L62.3,302.1c-0.7-5.7-6-9.7-11.6-9c-5.7,0.7-9.7,5.9-9,11.6l21.7,167.3c1.7,15.9,15,27.9,31,27.9h310c16,0,29.3-12,31-27.7
      L457.2,304C457.9,298.3,453.9,293.1,448.2,292.3z"
                        ></path>
                        <path
                          data-v-b3340884=""
                          d="M166.3,447.8V323.4c0-5.7-4.6-10.4-10.4-10.4c-5.7,0-10.4,4.6-10.4,10.4v124.4c0,5.7,4.6,10.4,10.4,10.4
      C161.6,458.2,166.3,453.6,166.3,447.8z"
                        ></path>
                        <path
                          data-v-b3340884=""
                          d="M353.3,447.8V323.4c0-5.7-4.6-10.4-10.4-10.4c-5.7,0-10.4,4.6-10.4,10.4v124.4c0,5.7,4.6,10.4,10.4,10.4
      C348.6,458.2,353.3,453.6,353.3,447.8z"
                        ></path>
                        <path
                          data-v-b3340884=""
                          d="M290.9,447.8V323.4c0-5.7-4.6-10.4-10.4-10.4s-10.4,4.6-10.4,10.4v124.4c0,5.7,4.6,10.4,10.4,10.4
      S290.9,453.6,290.9,447.8z"
                        ></path>
                        <path
                          data-v-b3340884=""
                          d="M228.6,447.8V323.4c0-5.7-4.6-10.4-10.4-10.4c-5.7,0-10.4,4.6-10.4,10.4v124.4c0,5.7,4.6,10.4,10.4,10.4
      C224,458.2,228.6,453.6,228.6,447.8z"
                        ></path>
                        <path
                          data-v-b3340884=""
                          d="M468.4,188.4h-43.6L278.7,46.1c1.8-3.9,2.8-8.1,2.8-12.7c0-16.8-13.8-30.5-30.7-30.5s-30.7,13.7-30.7,30.5
      c0,4.5,1,8.8,2.8,12.7L76.9,188.4H32.1C15,188.4,1,202.4,1,219.5v20.8c0,17.2,14,31.2,31.2,31.2h436.3c17.2,0,31.2-14,31.2-31.2
      v-20.8C499.6,202.4,485.6,188.4,468.4,188.4z M250.9,23.7c5.5,0,9.9,4.4,9.9,9.8c0,2.6-1.1,5-2.8,6.8c0,0,0,0-0.1,0.1
      c0,0,0,0.1-0.1,0.1c-1.8,1.8-4.2,2.8-7,2.8c-2.7,0-5.2-1.1-7-2.8c0,0,0-0.1-0.1-0.1c0,0,0,0-0.1-0.1c-1.7-1.8-2.8-4.1-2.8-6.8
      C241,28,245.4,23.7,250.9,23.7z M237.5,60.9c4,1.9,8.5,3.1,13.3,3.1c4.8,0,9.3-1.1,13.3-3.1L395,188.4H106.7L237.5,60.9z
       M478.8,240.3c0,5.7-4.7,10.4-10.4,10.4H32.1c-5.7,0-10.4-4.7-10.4-10.4v-20.8c0-5.7,4.7-10.4,10.4-10.4h436.3
      c5.7,0,10.4,4.7,10.4,10.4V240.3z"
                        ></path>
                      </g></svg
                  ></span>
                  Add to cart</a
                >
              </div>
            </div>
          </NuxtLink>
        </b-col>
      </b-container>
    </div>
  </section>
</template>

<script>
import { mapGetters } from 'vuex'
export default {
  props: {
    products: {
      type: Array,
      default:() => []
    }
  },
  computed: {
    ...mapGetters({
      ides: 'modules/products/ides',
    }),
    ides1() {
      return this.ides.slice(0,4)
    },
    ides2() {
      return this.ides.slice(4,8)
    }
  }
}
</script>

<style lang="scss">
.mt-60 {
  margin-top: 60px;

  .combo-icon {
    justify-content: center;
  }

  .recommend-food-content {
    width: 275px;
  }

  .title {
    color: #2f3e30;
    font-weight: 600;
    font-size: 18px;
    line-height: 1.5;
    text-align: center;
    cursor: pointer;
    margin-bottom: 15px;
  }
  .title:hover {
    color: #5c9963;
  }

  .recommend-food-main {
    justify-content: space-between;
    padding-left: 0px;
    padding-right: 0px;
  }

  .icon-shopping {
    background-color: #244263;
    border-radius: 50%;
    content: '';
    height: 40px;
    transition: background-color 0.2s ease-out;
    width: 40px;
    display: flex;
    align-items: center;
    text-align: center;
  }

  .icon-shopping:not(:last-child) {
    margin-right: 10px;
  }

  .icon-fix-re {
    fill: white;
    margin: 0 auto;
    width: 20px;
  }

  .recommend-foods:hover .img-idea {
    transform: translateY(-3px);
  }
}

.add-to-cart-wrap {
  align-items: center;
  margin-top: 20px;
  text-align: center;
}
.add-to-cart-wrap a {
  text-align: center;
  font-weight: 500;
  display: flex;
  align-items: center;
  justify-content: center;
  padding: 0.6em;
  background-color: transparent;
  color: #656766;
  transition: all ease-in-out 0.3s;
  border: 1px solid;
  border-color: #e4e4e4;
  font-size: 12px;
  text-transform: uppercase;
  border-radius: 24px;
  cursor: pointer;
  line-height: 1.4;
}

.padding-idea {
  padding-left: 30px;
  padding-right: 30px;
}

.recommend-foods:hover .padding-idea > a {
  background: #5c9863;
  color: white;
}


.recommend-foods:hover .qodef-svg {
  fill: #fff;
}
</style>
